...
namespace EVDealerSales.Business.Services
{
    public class PaymentService : IPaymentService
    {
        ...
        public async Task<PaymentIntentResponseDto> CreatePaymentIntentAsync(Guid orderId)
        {
            try
            {
                ...
                _logger.LogInformation("User {UserId} creating payment intent for order {OrderId}",
                ...
                var existingPaidPayment = invoice.Payments.FirstOrDefault(p => p.Status != PaymentStatus.Failed);
                ...
                var amountInCents = (long)(order.TotalAmount); // Convert to cents
                ...
                var paymentIntentData = new Dictionary<string, string>
                {
                    ...
                    { "metadata[customer_email]", order.Customer?.Email ?? "unknown" }
                };
                ...
                {
                    _logger.LogError("Stripe API error: {Error}", responseContent);
                    throw new InvalidOperationException($"Failed to create payment intent: {responseContent}");
                }

                var paymentIntent = JsonSerializer.Deserialize<StripePaymentIntent>(responseContent, new JsonSerializerOptions
                {
                ...
                {
                    throw new InvalidOperationException("Failed to parse payment intent response");
                }
                ...
                _logger.LogInformation("Payment intent {PaymentIntentId} created for order {OrderId}",
